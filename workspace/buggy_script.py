"""
buggy_script.py
A small inventory/order-total calculator with several intentional bugs,
for testing coding agents (Claude Code, Codex CLI, or the custom agent
built earlier in this conversation).

Expected behavior: given a list of items (name, price, quantity), compute
the subtotal, apply a discount code, add tax, and print a receipt.
"""
...
def apply_discount(subtotal, discount_code):
    ...
    if discount_code in discounts:
        return subtotal * (1 - discounts[discount_code])  
    return subtotal
